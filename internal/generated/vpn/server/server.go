...
// Vpner is de.telekomMMS.vpn interface.
type Vpner interface {
	// GetStatus is de.telekomMMS.vpn.GetStatus method.
	GetStatus() (status map[string]dbus.Variant, err *dbus.Error)
	// Connect is de.telekomMMS.vpn.Connect method.
	Connect(password string, server string) (err *dbus.Error)
	// Disconnect is de.telekomMMS.vpn.Disconnect method.
	Disconnect() (err *dbus.Error)
	// ListServers is de.telekomMMS.vpn.ListServers method.
	ListServers() (servers []string, err *dbus.Error)
}

// ExportVpn exports the given object that implements de.telekomMMS.vpn on the bus.
func ExportVpn(conn *dbus.Conn, path dbus.ObjectPath, v Vpner) error {
...
// UnexportVpn unexports de.telekomMMS.vpn interface on the named path.
func UnexportVpn(conn *dbus.Conn, path dbus.ObjectPath) error {
	return conn.Export(nil, path, InterfaceVpn)
}

// UnimplementedVpn can be embedded to have forward compatible server implementations.
type UnimplementedVpn struct{}

func (*UnimplementedVpn) iface() string {
	return InterfaceVpn
}

func (*UnimplementedVpn) GetStatus() (status map[string]dbus.Variant, err *dbus.Error) {
	err = &dbus.ErrMsgUnknownMethod
	return
}

func (*UnimplementedVpn) Connect(password string, server string) (err *dbus.Error) {
	err = &dbus.ErrMsgUnknownMethod
	return
}

func (*UnimplementedVpn) Disconnect() (err *dbus.Error) {
	err = &dbus.ErrMsgUnknownMethod
	return
}

func (*UnimplementedVpn) ListServers() (servers []string, err *dbus.Error) {
	err = &dbus.ErrMsgUnknownMethod
	return
}
